...
class ConceptKBExampleSampler:
    def __init__(self, concept_kb: ConceptKB, random_seed: int = 42):
    ...
    def sample_negative_examples(
        ...
        logger.info(
            f"Attempting to sample {n_neg_per_concept} negative examples per concept"
        )
        ...
        for neg_concept in neg_concepts:
            try:
                neg_examples = self.rng.choice(
                    neg_concept.examples, n_neg_per_concept, replace=False
                )

            except ValueError:  # Not enough negative examples
                logger.warning(
                    f"Not enough examples to sample from for concept {neg_concept.name}; using all examples"
                )
...
class ConceptKBTrainer(ConceptKBForwardBase):
    def __init__(
        ...
        feature_pipeline: ConceptKBFeaturePipeline = None,
        wandb_run: Run = None,
    ):
        """
    ...
    def train(
        ...
        if concepts is None:
            if leaf_nodes_only:
                ...
                logger.info(
                    "Training all concepts in ConceptKB, including internal nodes"
                )
        ...
        for epoch in range(1, n_epochs + 1):
            ...
            for i, batch in enumerate(
                tqdm(train_dl, desc=f"Epoch {epoch}/{n_epochs}"), start=1
            ):
                ...
                self.log(
                    {
                        ...
                        "val_acc": outputs["val_acc"],
                        "epoch": epoch,
                    }
                )
                logger.info(
                    f'Validation loss: {outputs["val_loss"]}, Validation accuracy: {outputs["val_acc"]}'
                )

        # Construct return dictionary
        best_ckpt_epoch = np.argmin(val_losses) if val_losses else None
        best_ckpt_path = (
            self._get_ckpt_path(ckpt_dir, ckpt_fmt, best_ckpt_epoch)
            if val_losses
            else None
        )
    ...
    def train_concept(
        ...
        stopping_condition: Literal[
            "until_correct", "n_epochs", "validation"
        ] = "until_correct",
        until_correct_examples: list[ConceptExample] = [],
        min_prob_margin: float = 0.1,
        ...
        **train_kwargs,
        ...
        # Train for a fixed number of epochs or until examples are predicted correctly
            ...
            if sample_only_siblings:
                ...
                neg_concepts = [
                    c for c in self.concept_kb.leaf_concepts if c != concept
                ]
            else:
            ...
            if sample_all_negatives:
                neg_examples, neg_concept_names = self.sampler.get_all_examples(
                    neg_concepts
                )
            else:
                neg_examples, neg_concept_names = self.sampler.sample_negative_examples(
                    len(concept.examples), neg_concepts
                )
            ...
            if any(feature_path is None for feature_path in all_feature_paths):
                raise RuntimeError(
                    "All examples must have image_features_path set to train individual Concept"
                )

            until_correct_example_paths = [
                ex.image_features_path for ex in until_correct_examples
            ]
            ...
            val_ds = FeatureDataset(
                until_correct_example_paths,
                [concept.name] * len(until_correct_example_paths),
            )
            ...
            if stopping_condition == "n_epochs":
                ...
                while True:
                    ...
                    results = self.train(
                        train_ds,
                        None,
                        n_epochs=n_epochs_between_predictions,
                        **train_kwargs,
                    )
                    ...
                    all_scores = torch.stack(
                        [pred["predictors_scores"] for pred in predictions]
                    )
                    normalized_scores = F.softmax(all_scores, dim=1)
                    logger.debug(f"Normalized scores: {normalized_scores}")

                    values, indices = normalized_scores.topk(
                        2, dim=1
                    )  # Each of values, indices are (n_examples, 2) tensors

                    logger.debug(f"Top two scores: {values}")
                    logger.debug(
                        f"Target scores: {normalized_scores[:, target_concept_index]}"
                    )

                    if not (
                        indices[:, 0] == target_concept_index
                    ).all():  # Not all examples are correctly predicted
                        continue

                    prob_margins = (
                        values[:, 0] - values[:, 1]
                    )  # Top prediction prob - 2nd prediction prob
                    logger.debug(
                        f"Concept {concept.name} predicted examples with margins {prob_margins}"
                    )
                ...
                logger.info(
                    f"Concept {concept.name} correctly predicted examples with margins {prob_margins} after {curr_epoch} loops"
                )
    ...
    def validate(
        self, val_dl: DataLoader, leaf_nodes_only_for_accuracy=True, **forward_kwargs
    ):
        ...
        for batch in tqdm(val_dl, desc="Validation"):
            ...
            scores = torch.tensor(
                [output.cum_score for output in outputs["predictors_outputs"]]
            )
            ...
            predicted_concept_outputs.append(
                outputs["predictors_outputs"][pred_ind.item()]
            )
