...
def bbox_from_mask(
    masks: Union[torch.Tensor, np.ndarray], use_dim_order: bool = False
) -> Union[torch.IntTensor, np.ndarray]:
    """
    ...
    left_inds = (
        masks.any(dim=1).int().argmax(dim=1)
    )  # Get the first column with nonzero values; (n,)
    bottom_inds = (
        masks.shape[1] - 1 - masks.flip(dims=(1,)).any(dim=2).int().argmax(dim=1)
    )  # Reverse rows to get last row with nonzero vals; (n,)
    right_inds = (
        masks.shape[2]
        - 1
        - (  # Since reversing, subtract from total width
            masks.flip(dims=(2,))
            .any(dim=1)  # Reverse columns to get last column with nonzero vals; (n,h)
            .int()  # Argmax not implemented for bool, so convert back to int
...
class Localizer:
    ...
    def rembg_ground(self, img: Image) -> torch.IntTensor:
        ...
        for i, coord in enumerate(bbox):
            if i < 2 and coord > 0:
                bbox[i] -= 1
            elif (i == 2 and coord + 1 < mask.shape[1]) or (
                i == 3 and coord + 1 < mask.shape[0]
            ):
                bbox[i] += 1
    ...
    def rembg_mask(self, img: Image) -> torch.BoolTensor:
        ...
        mask = remove(
            img, session=self.rembg_session, post_process_mask=True, only_mask=True
        )
    ...
    def desco_ground(
        ...
        img: Image,
        caption: str,
        tokens_to_ground: Union[str, list[str]],
        ...
        predictions = self.desco.compute_prediction(
            np.array(img), caption, specified_tokens=tokens_to_ground
        )
        ...
        bboxes = (
            bboxes.round().int()
        )  # (n_detected, 4); Desco returns fractional bbox predictions
    ...
    def desco_mask(
        ...
        """
        bboxes = self.desco_ground(
            img, caption, tokens_to_ground, conf_thresh
        ).numpy()  # SAM takes numpy bbox

        if len(bboxes) == 0:
            return torch.zeros(
                0, img.size[1], img.size[0]
            ).bool()  # (0, h, w); Image.Image has size (w, h)
    ...
    def localize(
        ...
        if caption:
            ...
            logger.info(
                f'Localizing with DesCo to ground "{tokens_to_ground}" with caption "{caption}"'
            )
