...
def build_localizer_and_segmenter(sam: Sam, desco):
    return LocalizerAndSegmenter(Localizer(sam, desco), Segmenter(sam))


@dataclass
class LocalizeAndSegmentOutput:
    input_image: Image = field(default=None, metadata={"description": "Input image"})

    input_image_path: str = field(
        default=None, metadata={"description": "Path corresponding to input image"}
    )

    input_kwargs: dict = field(
        default=None, metadata={"description": "Input keyword arguments"}
    )

    part_masks: torch.BoolTensor = field(
        default=None,
        metadata={
            "description": "Boolean array of shape (n_detections, h, w) representing the segmentation masks of the parts"
        },
    )
    ...
    part_names: list[str] = field(
        default=None,
        metadata={
            "dsecription": "List of part names corresponding to part_masks. None if part regions are unnamed (e.g. if segmented without grounding by name)."
        },
    )
    ...
    rle_part_masks: list[dict] = field(
        default=None,
        metadata={
            "description": "RLE-encoded masks output by pycocotools.mask.encode; currently unused"
        },
    )
    ...
    localized_bbox: torch.IntTensor = field(
        default=None,
        metadata={
            "description": "Bounding box of the localized concept in XYXY format with shape (4,)"
        },
    )
    ...
    part_crops: list[Image] = field(
        default=None,
        metadata={
            "description": "List of cropped part images, if return_crops is True"
        },
    )
    ...
    localized_part_bboxes: torch.IntTensor = field(
        default=None,
        metadata={
            "description": "Tensor of shape (n_part_detections, 4) of bboxes of localized parts in XYXY format, if concept_parts is provided."
        },
    )
...
class LocalizerAndSegmenter:
    ...
    def localize_and_segment(
        ...
        caption = (
            self._get_parts_caption(concept_name, concept_parts)
            if concept_parts
            else concept_name
        )
        bboxes = self.localizer.localize(
            image, caption=caption, tokens_to_ground=[concept_name]
        )

        if len(bboxes) == 0:  # Fall back to rembg if DesCo fails
            if concept_name:
                logger.warning(
                    "Failed to ground concept with caption; retrying with rembg"
                )
        ...
        if concept_parts:
            ...
            cropped_image = self.segmenter.crop(
                image, bboxes[0], remove_background=remove_background
            )  # Crop around localized concept
            part_masks = self.localizer.desco_mask(
                cropped_image, caption=caption, tokens_to_ground=concept_parts
            )  # (n_detections, h, w)
            ...
            if remove_background:  # Crop had background removed, so just extract mask
                crop_foreground_mask = (
                    pil_to_tensor(cropped_image).bool().sum(dim=0).bool()
                )

            else:  # Don't remove background
                crop_foreground_mask = torch.ones(
                    cropped_image.size[1], cropped_image.size[0], dtype=torch.bool
                )

            for i, part_mask in enumerate(part_masks):
                full_part_mask = torch.zeros(
                    image.size[1], image.size[0], dtype=torch.bool
                )
                full_part_mask[y1:y2, x1:x2] = part_mask & crop_foreground_mask

                if full_part_mask.sum() == 0:
                    logger.warning(
                        f"Part mask {i} is empty after intersecting with foreground; skipping"
                    )
                    continue
            ...
            part_masks = self.segmenter.segment(
                image, bboxes[0], remove_background=remove_background
            )
        ...
        if return_crops:
            if use_bbox_for_crops:
                ...
                part_crops = [
                    to_pil_image(
                        draw_bounding_boxes(
                            image_t, bbox[None, ...], width=3, colors="red"
                        )
                    )
                    for bbox in bboxes
                ]
                ...
                part_crops = self.segmenter.crops_from_masks(
                    image, part_masks, only_mask=remove_background
                )
            ...
            for i, crop in enumerate(part_crops):
                if crop.size[0] == 0 or crop.size[1] == 0:
                    logger.warning(
                        f"Part crop {i} has a zero-dimension; filtering out part mask and crop"
                    )
                    continue
            ...
            output.part_masks = (
                torch.stack(filtered_part_masks)
                if filtered_part_masks
                else torch.tensor([])
            )
